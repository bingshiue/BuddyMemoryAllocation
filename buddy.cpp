...
static void
_dump(struct buddy * self, int index , int level, void (*func)(const char*)) {
  char str[64];
  switch (self->tree[index]) {
  case NODE_UNUSED:
    sprintf(str, "(%d:%d)", _index_offset(index, level, self->level) , 1 << (self->level - level));
    func(str);
    break;
  case NODE_USED:
    sprintf(str, "[%d:%d]", _index_offset(index, level, self->level) , 1 << (self->level - level));
  ...
  case NODE_FULL:
    func("{");
    _dump(self, index * 2 + 1 , level+1, func);
    _dump(self, index * 2 + 2 , level+1, func);
    func("}");
    break;
  default:
    func("(");
...
void
buddy_dump_f(struct buddy * self, void (*func)(const char*)) {
  _dump(self, 0 , 0, func);
  func("\n");
}

static void default_dump_func(const char* str) {
  printf(str);
}

void buddy_dump(struct buddy * self) {
  _dump(self, 0 , 0, &default_dump_func);
  default_dump_func("\n");
}
